  ...
  return (
    <section className="featured-teachers-section">
      ...
      <div className="ft-carousel">
        <button className="ft-nav ft-prev" onClick={() => go(page - 1)} aria-label="Trước">‹</button>
          ...
          <div className="ft-track" style={{ transform: `translateX(-${page * 100}%)` }}>
            {slides.map((group, idx) => (
              <div className="ft-slide" key={idx}>
                {group.map((teacher, i) => (
                  <div className="ft-cardWrap" key={(teacher.name || "t") + i}>
                    <TeacherCard {...teacher} />
                  </div>
                ))}
              </div>
        ...
        <button className="ft-nav ft-next" onClick={() => go(page + 1)} aria-label="Sau">›</button>
      </div>
