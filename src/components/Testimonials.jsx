...
export default function Testimonials({ items = [] }) {
  ...
  const [perView, setPerView] = useState(
    typeof window === "undefined" ? 3 : calcPerView(window.innerWidth)
  );
  ...
  return (
    <section className="testimonials-section">
      <h2 className="ts-title">
        <span>Đánh giá về giáo viên của</span><br/>PHỤ HUYNH VÀ HỌC SINH
      </h2>

      <div className="ts-carousel">
        <button className="ts-nav ts-prev" onClick={() => go(page - 1)}>‹</button>
        <div className="ts-viewport">
          <div className="ts-track" style={{ transform: `translateX(-${page * 100}%)` }}>
            {slides.map((group, gi) => (
              <div className="ts-slide" key={gi}>
                {group.map((t, i) => (
                  <div className="ts-cardWrap" key={(t.name || "c")+i}>
                    <Card {...t} />
                  </div>
                ))}
              </div>
        ...
        <button className="ts-nav ts-next" onClick={() => go(page + 1)}>›</button>
      </div>
