...
function CopyInput({ label, value = "", editable = false }) {
  ...
  const doCopy = async () => {
    try { await navigator.clipboard.writeText(val || ""); setCopied(true); setTimeout(() => setCopied(false), 1200); } catch (_) {}
  };
  ...
  return (
    <label>
      ...
      <div className="topup__copywrap">
        ...
        <button type="button" className="topup__copybtn" onClick={doCopy}>{copied ? "Đã copy" : "Copy"}</button>
      </div>
    </label>
  );
}
...
const userId = "123456789", username = "Nguyen Van A";
...
export default function TopUp({ qrImage, bankInfo }) {
  ...
  return (
    <div className="topup">
      ...
      <NavLink to="." className={`topup__radio ${!isEW ? "topup__radio--active" : ""}`}>
        Ngân hàng
      </NavLink>
      <NavLink to="../ewallet" className={`topup__radio ${isEW ? "topup__radio--active" : ""}`}>
        Ví điện tử
      </NavLink>
    </div>
      ...
      <section className="topup__panel">
        ...
        <div className="topup__form-grid">
          ...
          <div className="topup__form">
            ...
            <CopyInput label="Nội dung chuyển tiền:" value={`${bank.holder} - ${userId} - ${username}` } editable />
          ...
          <aside className="topup__qr-mini">
            ...
            <div className="topup__qr-mini-meta">
              <div><strong>Ngân hàng thụ hưởng</strong><br/>{bank.bank}</div>
              <div><strong>Số TK</strong><br/>{bank.accountMasked || bank.account}</div>
              <div><strong>Tên chủ tài khoản</strong><br/>{bank.holder}</div>
            </div>
          </aside>
        </div>
      </section>
